...
while true
    ...
    % North cell | Above cell
    if (i>1 && i<=nrows)  % check index is within the map size
        i_nb = i-1;   % Row index of the neighbor cell
        j_nb = j;   % Column index of the neighbor cell
        if (map(i_nb,j_nb)~=2 && map(i_nb,j_nb)~=3 && map(i_nb,j_nb)~=5 && g(i_nb,j_nb)>(g(i,j)+H(i_nb,j_nb)-H(i,j)))
    ...
    % West cell | Left cell
    if (j>1 && j<=ncols) % check index is within the map size
